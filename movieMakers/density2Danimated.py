...
species=int(sys.argv[11])		#1 = do total pop only; 2 = two species
...
while infile:
	...
	if i==xyzSize[0]*xyzSize[1]*xyzSize[2]: # end of a timestep
      ...
      #print( 'Toss %d'%currTStep )
			...
			imshow(sliceTOTPOP/sliceMax,cmap=myMap,vmin=0, vmax=1, 
				aspect=myAspect)

			#Create the colorbar
			CS3 = imshow(sliceTOTPOP.T/sliceMax,vmin=0,vmax=1,cmap=myMap,aspect=myAspect)			#pcolor() sucks this is way better
			cb=colorbar(CS3)
