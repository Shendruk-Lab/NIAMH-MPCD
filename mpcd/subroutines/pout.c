...
void stateinput( inputList in,spec SP[],bc WALL[],specSwimmer SS,outputFlagsList out,kinTheory theorySP[],kinTheory theoryGl,FILE *fsynopsis ) {
	...
	if( out.SYNOUT == OUT ) {
		...
		for( i=0; i<NBC; i++ ) {
			...
			fprintf( fsynopsis,"\tSurface mode: %d\n",WALL[i].SURFMODE );
			fprintf( fsynopsis,"\tMode %d parameters:\n",TRAD_SURF );
			...
			fprintf( fsynopsis,"\tMode %d parameters:\n",VERT_SURF );
			fprintf( fsynopsis,"\t\tSmoothing parameter: %lf\n",WALL[i].SMOOTH_VERT );
			fprintf( fsynopsis,"\t\tNumber of vertices: %d\n",WALL[i].NUMVERT );
			for( j=0; j<WALL[i].NUMVERT; j++ ) fprintf( fsynopsis,"\t\t\t[%lf, %lf, %lf]\n",WALL[i].VERTICES[j][0],WALL[i].VERTICES[j][1],WALL[i].VERTICES[j][2] );
